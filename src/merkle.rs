...
impl IncrementalMerkleTree {
    ...
    pub fn verify(&self, proof: &MerkleProof) -> crate::Result<()> {
        let is_valid = Self::verify_proof(&self.root, &proof.leaf.key, &proof.leaf.value, proof);
...
mod tests {
    ...
    fn test_multiple_inserts() {
        ...
        assert!(IncrementalMerkleTree::verify_proof(
            &root1, b"key1", b"value1", &proof1
        ));
        assert!(IncrementalMerkleTree::verify_proof(
            &root2, b"key2", b"value2", &proof2
        ));
        assert!(IncrementalMerkleTree::verify_proof(
            &root3, b"key3", b"value3", &proof3
        ));
