...
use cardano_lsm::{Key, LsmConfig, LsmTree, Result as LsmResult, Value};
...
impl TestRunner {
    fn new(config: &TestConfig, test_name: &str) -> LsmResult<Self> {
        ...
        let temp_dir =
            std::env::temp_dir().join(format!("conformance_{}_{}_{}", test_name, pid, timestamp));
    ...
    fn execute_operation(&mut self, op: &Operation) -> OperationResult {
        match op {
            Operation::Insert { key, value } => {
                ...
                match self
                    .tree
                    .insert(&Key::from(&key_bytes), &Value::from(&value_bytes))
                {
            ...
            Operation::Range { from, to } => {
                let from_bytes = general_purpose::STANDARD
                    .decode(from)
                    .expect("Invalid base64 from key");
                let to_bytes = general_purpose::STANDARD
                    .decode(to)
                    .expect("Invalid base64 to key");
            ...
            Operation::Rollback { snapshot_id } => match self.snapshots.get(snapshot_id) {
                Some(snapshot) => match self.tree.rollback(snapshot.clone()) {
                    Ok(()) => OperationResult::OkUnit(None),
                    Err(e) => OperationResult::Err(format!("{}", e)),
                },
            ...
            Operation::Compact => match self.tree.compact() {
                Ok(()) => OperationResult::OkUnit(None),
                Err(e) => OperationResult::Err(format!("{}", e)),
            },
        }
...
fn run_conformance_test(test_path: &Path) -> Result<(), String> {
    ...
    // Load test case
    let test_json =
        fs::read_to_string(test_path).map_err(|e| format!("Failed to read test file: {}", e))?;
            ...
            match (expected, actual) {
                (OperationResult::OkRange(exp_entries), OperationResult::OkRange(act_entries)) => {
                    ...
                    if exp_entries.len() != act_entries.len() {
                        ...
                        let exp_keys: std::collections::HashSet<_> =
                            exp_entries.iter().map(|(k, _)| k).collect();
                        let act_keys: std::collections::HashSet<_> =
                            act_entries.iter().map(|(k, _)| k).collect();
                        ...
                        for (idx, (k, _)) in exp_entries.iter().enumerate() {
                            if !act_keys.contains(k) {
                                println!(
                                    "    MISSING key from actual (expected at index {}): {}",
                                    idx, k
                                );
                            }
                        }
                        ...
                        for (idx, (exp, act)) in
                            exp_entries.iter().zip(act_entries.iter()).enumerate()
                        {
                            if exp.0 != act.0 {
                                println!(
                                    "    Difference at index {}: expected key='{}', actual key='{}'",
                                    idx, exp.0, act.0
                                );
                                if idx > 3 {
...
fn conformance_tests() {
    ...
    if test_files.is_empty() {
        ...
        if !status.success() {
            panic!(
                "Failed to generate conformance tests. Run 'just gen-conformance 100' manually."
            );
        }
        ...
        if test_files_after.is_empty() {
            panic!(
                "Test generation succeeded but no test files found. Check conformance-generator/conformance-tests/"
            );
        }
    }
    ...
    for test_path in &test_files {
        match run_conformance_test(test_path) {
            ...
            Err(e) => {
                failed += 1;
                errors.push((
                    test_path.file_stem().unwrap().to_str().unwrap().to_string(),
                    e,
                ));
            }
        }
